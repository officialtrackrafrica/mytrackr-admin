...
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { SearchNormal, Add, Trash, Edit2 } from "iconsax-react";
import { useMessages, useDeleteMessage, useBulkDeleteMessages, type GetMessagesParams } from "./apis/useMessages";
import { toast } from "sonner";
import { Link, useNavigate } from "react-router-dom";
...
export const EmailsNotifications = () => {
  ...
  const { mutate: deleteSingle } = useDeleteMessage();
  const { mutate: deleteBulk, isPending: isDeletingBulk } = useBulkDeleteMessages();
  ...
  // 👉 2. Bulk Delete Handler
  const handleBulkDelete = () => {
    if (!selectedItems.length) return;
    deleteBulk(selectedItems, {
      onSuccess: () => {
        ...
        <div className="px-6 py-3 bg-slate-50 border-b border-slate-200 flex items-center justify-between text-xs font-semibold text-[#475467]">
          <div className="flex items-center gap-3">
            <input 
              ...
              checked={messages.length > 0 && selectedItems.length === messages.length}
              onChange={toggleSelectAll}
              className="rounded border-slate-300 text-[#0F4BAB] focus:ring-[#0F4BAB] cursor-pointer" 
            />
            <span>Users</span>
          </div>
          {selectedItems.length > 0 && (
            <button 
              ...
              className="text-red-600 hover:text-red-700 flex items-center gap-1 font-medium disabled:opacity-50"
            >
              <Trash size="16" color="currentColor" /> 
              {isDeletingBulk ? "Deleting..." : `Delete (${selectedItems.length})`}
            </button>
          )}
        </div>
            ...
            messages.map((item: any) => {
              ...
              return (
                <div 
                  key={id} 
                  onClick={() => navigate(`/dashboard/content/emails/${id}`)}
                  className="px-6 py-4 flex items-center justify-between gap-4 hover:bg-slate-50/50 transition-colors cursor-pointer group"
                >
  ...
  To: {Array.isArray(item.recipients) && item.recipients.length > 0
    ? item.recipients.join(", ")
    : item.recipientGroup?.replace("_", " ") || "Unknown group"}
</p>
                      <h4 className="text-sm font-bold text-slate-900">{item.subject || "No Subject"}</h4>
                      <p className="text-xs text-[#475467] line-clamp-1 max-w-2xl">{item.body || "No preview available..."}</p>
                    </div>
                  ...
                  <div className="flex items-center gap-4">
                    <span className="text-xs text-[#475467] whitespace-nowrap group-hover:hidden transition-all">
                      {timeString}
                    </span>
                    <div className="hidden group-hover:flex items-center gap-2 transition-all">
                      <button 
                        onClick={(e) => {
                          e.stopPropagation();
                          navigate(`/dashboard/content/emails/compose?edit=${id}`); // Assuming you use compose for edits
                        }} 
                        className="p-1.5 text-slate-400 hover:bg-slate-200 hover:text-[#0F4BAB] rounded-md transition-colors"
                      >
                        <Edit2 size="16" />
                      </button>
                      <button 
        ...
        <div className="p-4 border-t border-slate-200 flex items-center justify-between text-xs text-[#475467]">
          <Button 
            variant="outline" size="sm" className="text-[#475467]"
            onClick={() => setPage(p => Math.max(1, p - 1))}
            disabled={page === 1 || isLoading}
          >
            Previous

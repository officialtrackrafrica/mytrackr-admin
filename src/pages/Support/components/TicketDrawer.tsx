...
//                   </p>
//                   <img src={ticket.attachmentUrl} alt="" className="w-48 h-48"/>
//                 </div>
//               </div>
...
export const TicketDrawer = ({ isOpen, onClose, ticketId, initialTicket }: TicketDrawerProps) => {
  ...
  // 👉 1. Look for the "messages" array and filter out the initial ticket duplicate
  const rawMessages = data?.messages || ticket?.messages || data?.replies || ticket?.replies || [];
  const replies = rawMessages.filter((reply: any) => reply.id !== ticket.id);
      ...
      <div className="flex flex-col h-full overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-8">
          ...
          <div className="flex items-start justify-between border-b border-slate-100 pb-6 shrink-0">
            ...
            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium capitalize ${statusStyles[statusStr] || statusStyles.open}`}>
              <span className={`w-1.5 h-1.5 rounded-full ${dotColor[statusStr as keyof typeof dotColor] || dotColor.open}`}></span>
              {statusStr}
            </span>
          </div>
            ...
            <>
              ...
              <div className="flex gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-[#475467] leading-relaxed whitespace-pre-wrap mb-3">
                    {ticket?.description || ticket?.message}
                  </p>
                  {ticket?.attachmentUrl && (
                    <img src={ticket.attachmentUrl} alt="Attachment" className="w-48 h-48 object-cover rounded-lg border border-slate-200"/>
                  )}
                </div>
              </div>
              ...
              {replies.map((reply: any) => (
                <div key={reply.id || reply._id} className="flex gap-4">
                  <Avatar className="h-8 w-8 mt-1 border border-slate-200 shrink-0">
                    {/* 👉 2. Map based on senderType */}
                    {reply.senderType === "admin" ? (
                      <AvatarFallback className="bg-blue-600 text-white text-xs font-bold">M</AvatarFallback>
                    ) : (
                      <AvatarFallback className="bg-purple-100 text-purple-700 text-xs font-bold">
                        {(reply.senderName || "U").charAt(0).toUpperCase()}
                      </AvatarFallback>
                    )}
                  </Avatar>
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      {/* 👉 3. Map based on senderType and senderName */}
                      <span className="font-semibold text-slate-900 text-sm">
                        {reply.senderType === "admin" ? "MyTrackr Support" : (reply.senderName || "Customer")}
                      </span>
                      <span className="text-xs text-[#475467]">
                        {new Date(reply.createdAt).toLocaleString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: 'numeric'})}
                      </span>
                    ...
                    {reply.attachmentUrl && (
                      <img src={reply.attachmentUrl} alt="Reply Attachment" className="w-48 h-48 object-cover rounded-lg border border-slate-200"/>
                    )}
                  </div>
                </div>
              ))}

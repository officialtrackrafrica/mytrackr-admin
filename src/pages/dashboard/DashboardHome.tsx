...
import { AdminLayout } from '@/components/layout/AdminLayout';
// 👉 Swapped useFinancialSummary for usePlatformRevenue
import { useAdminStats, usePlatformRevenue, usePlatformStats, useRegistrationTrends, type PlatformStats } from '@/components/hooks/useAdminStats';
import { ChurnChart } from '../Subscriptions/components/ChurnChart';
...
export const DashboardHome = () => {
  ...
  // 👉 Fetch revenue using the new hook, passing both dates and period
  const { data: revenueData, isLoading: isRevenueLoading } = usePlatformRevenue({
    ...dateParams,
    period: apiPeriod
  });
  ...
  // 👉 Map API data to the MRR Chart securely using useMemo
  const mrrData = useMemo(() => {
    ...
    // Sort chronologically just in case the API returns them out of order
    const sortedData = [...revenueData.data].sort((a, b) => a.period.localeCompare(b.period));

    return sortedData.map((item: any) => {
      let label = item.period;
      
      // Dynamic labeling based on the active API period
      if (apiPeriod === 'month' && item.period.includes('-')) {
        const [year, month] = item.period.split('-');
        const date = new Date(parseInt(year), parseInt(month) - 1);
        label = date.toLocaleString('default', { month: 'short' }).toUpperCase();
      } else if (apiPeriod === 'week' && item.period.includes('-')) {
        label = `Wk ${item.period.split('-')[1]}`;
      } else if (apiPeriod === 'day' && item.period.split('-').length === 3) {
        const parts = item.period.split('-');
  ...
  }, [revenueData, apiPeriod]);

  // 👉 Grab the top-level total from the new response format
  const currentRevenue = revenueData?.totalRevenue || 0;

  const stats: PlatformStats = platformStats?.stats || platformStats || {};
  const globalChurnRate = stats.churnRate || 0;
  const plans = stats.planSubscriptionStats || [];
  const totalActiveSubs = stats.activeSubscriptions || 1; 
